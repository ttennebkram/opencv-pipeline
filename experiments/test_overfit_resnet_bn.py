#!/usr/bin/env python3
"""
ResNet with BatchNorm - the standard modern combination.
"""
import json
...
class ResBlock(nn.Module):
    """Residual block with BatchNorm (standard ResNet style)."""
...
def main():
    ...
    print(f"Parameters: {params:,} (ResNet + BatchNorm)", flush=True)
    print(flush=True)

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)

    print(f"{'Epoch':>5} | {'Loss':>12} | {'Pixel Err':>10}", flush=True)
    print("-" * 40, flush=True)
    ...
    with torch.no_grad():
        for imgs, corners in loader:
            ...
            for i in range(min(3, len(out))):
                ...
                print(f"Sample {i}: {err_i:.1f}px", flush=True)
                print(f"  GT:   {gt.round(2)}", flush=True)
                print(f"  Pred: {pr.round(2)}", flush=True)
