...
public class BitPlanesColorNode extends ProcessingNode {
    ...
    public void serializeProperties(JsonObject json) {
        ...
        for (int i = 0; i < 8; i++) {
            redEnabled.add(bitEnabled[0][i]);
            redGain.add(bitGain[0][i]);
        }
        json.add("redBitEnabled", redEnabled);
        json.add("redBitGain", redGain);

        // Serialize Green channel
        JsonArray greenEnabled = new JsonArray();
        JsonArray greenGain = new JsonArray();
        for (int i = 0; i < 8; i++) {
            greenEnabled.add(bitEnabled[1][i]);
            greenGain.add(bitGain[1][i]);
        }
        json.add("greenBitEnabled", greenEnabled);
        json.add("greenBitGain", greenGain);

        // Serialize Blue channel
        JsonArray blueEnabled = new JsonArray();
        JsonArray blueGain = new JsonArray();
        for (int i = 0; i < 8; i++) {
            blueEnabled.add(bitEnabled[2][i]);
            blueGain.add(bitGain[2][i]);
        }
        json.add("blueBitEnabled", blueEnabled);
        json.add("blueBitGain", blueGain);
    }
    ...
    public void deserializeProperties(JsonObject json) {
        ...
        if (json.has("redBitEnabled")) {
            ...
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitEnabled[0][i] = arr.get(i).getAsBoolean();
            }
        }
        if (json.has("redBitGain")) {
            JsonArray arr = json.getAsJsonArray("redBitGain");
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitGain[0][i] = arr.get(i).getAsDouble();
            }
        }

        // Deserialize Green channel
        if (json.has("greenBitEnabled")) {
            JsonArray arr = json.getAsJsonArray("greenBitEnabled");
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitEnabled[1][i] = arr.get(i).getAsBoolean();
            }
        }
        if (json.has("greenBitGain")) {
            JsonArray arr = json.getAsJsonArray("greenBitGain");
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitGain[1][i] = arr.get(i).getAsDouble();
            }
        }

        // Deserialize Blue channel
        if (json.has("blueBitEnabled")) {
            JsonArray arr = json.getAsJsonArray("blueBitEnabled");
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitEnabled[2][i] = arr.get(i).getAsBoolean();
            }
        }
        if (json.has("blueBitGain")) {
            JsonArray arr = json.getAsJsonArray("blueBitGain");
            for (int i = 0; i < 8 && i < arr.size(); i++) {
                bitGain[2][i] = arr.get(i).getAsDouble();
            }
        }
    }
}
